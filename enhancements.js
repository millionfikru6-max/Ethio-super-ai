...
(function () {
  ...
  window.EthioRefreshFavoriteButtons = markFavoriteButtons;
  ...
  /* Pre-fill the Account Settings fields with the logged-in user's current
     values (they previously always opened blank, forcing a full retype). */
  function prefillSettingsFields() {
    ...
    if (window.currentUser) {
      ...
      if (nameEl && !nameEl.value) nameEl.value = window.currentUser.name || "";
    }
  }

  /* ==================================================================
   * 6b. USER PROFILE — "Recent Tracks" was static placeholder markup;
   *     wire it to the projects the user has actually saved in the studio.
   * ================================================================== */
  function renderProfileTracks() {
    const list = $("#profile-tracks-list");
    if (!list) return;
    const projects = (typeof savedProjects !== "undefined" ? savedProjects : []);
    if (!projects.length) {
      list.innerHTML = `<p>No tracks created yet. Start creating in the Music Studio!</p>`;
      return;
    }
    list.innerHTML = projects.slice().reverse().slice(0, 8).map(p => `
      <div class="library-item">
        <div class="item-info"><h4>${escapeHtml(p.name)}</h4><p>${escapeHtml(p.genre || "No genre")} · ${escapeHtml(p.mood || "No mood")}</p></div>
        <div class="item-actions">
          <button class="action-icon" data-load-project="${p.id}" aria-label="Open in studio">▶</button>
        </div>
      </div>
    `).join("");
    $all("[data-load-project]", list).forEach(btn => {
      btn.addEventListener("click", () => {
        window.switchWorkspace("music-studio");
        window.loadProject(Number(btn.dataset.loadProject));
      });
    });
  }
  ...
  function buildSearchIndex() {
    ...
    const workspaces = {
      ...
      "ai-tools": "AI Tools", settings: "Settings", profile: "Profile"
    };
    Object.keys(workspaces).forEach(w => items.push({
      group: "Go to", label: workspaces[w], icon: "🧭", action: () => window.switchWorkspace(w)
    }));
    // Prefer the fuller catalog from platform-expansion.js when present.
    const catalog = (window.EthioMusicPlatform && window.EthioMusicPlatform.TRACKS) || TRACK_CATALOG;
    Object.keys(catalog).forEach(id => items.push({
      group: "Tracks", label: catalog[id].title, sub: catalog[id].artist, icon: "🎵",
      action: () => { window.switchWorkspace("discovery"); window.playTrack(id); }
    }));
    (window.EthioAITools?.tools || []).forEach(t => items.push({
      group: "AI Tools", label: t.name, icon: t.icon,
      action: () => { window.switchWorkspace("ai-tools"); setTimeout(() => window.EthioAITools.openTool(t.id), 50); }
    }));
  ...
  window.EthioMarkdown = { render: renderMarkdown, highlight: highlightCode, escapeHtml };
  ...
  function init() {
    ...
    // Keep favourite hearts / saved-project list / library tab / profile
    // in sync whenever the user switches workspace (existing function,
    ...
    window.switchWorkspace = function (workspace) {
      ...
      if (workspace === "profile") renderProfileTracks();
      if (workspace === "settings") prefillSettingsFields();
      closeAllOverlays();
    };
  }
