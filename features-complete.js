...
// ==================== LYRICS & WAVEFORM ====================
function esaTrackMeta(trackId) {
  const catalog = window.EthioMusicPlatform;
  if (catalog && typeof catalog.trackMeta === "function") return catalog.trackMeta(trackId);
  return { title: trackId, artist: "Unknown Artist", cover: "./profile.png" };
}

function esaHashStr(str) {
  let h = 0;
...
function showLyrics(trackId) {
  const meta = esaTrackMeta(trackId);
  const catalog = window.EthioMusicPlatform;
  const lyrics = catalog && catalog.LYRICS ? catalog.LYRICS[trackId] : null;
  const lyricsModal = document.createElement("div");
  lyricsModal.className = "modal";
  lyricsModal.innerHTML = `
    <div class="modal-content">
      <button class="modal-close" onclick="this.closest('.modal').remove()">×</button>
      <h2>Lyrics — ${meta.title}</h2>
      <div class="lyrics-content">
        ${lyrics
          ? `<p style="white-space: pre-line;">${lyrics}</p>`
          : `<p>🎵 ${meta.title}</p><p style="color: var(--text-secondary);">Lyrics aren't available for this track yet.</p>`}
      </div>
    </div>
  `;
...
function drawWaveform(trackId) {
  ...
  // Deterministic per-track pattern (seeded by track id) so the same
  // track always renders the same waveform shape instead of a fresh
  // random pattern on every open.
  let seed = esaHashStr(trackId || "track") || 1;
  const rand = () => {
  ...
  for (let i = 0; i < canvas.width; i += 5) {
    const height = 0.15 * canvas.height + rand() * canvas.height * 0.7;
  ...
  artworkModal.innerHTML = `
    <div class="modal-content">
      ...
      <div style="text-align: center;">
        <img src="${meta.cover}" alt="Album Art" style="max-width: 300px; border-radius: 12px; margin: 20px 0;">
        <p>${meta.title} — ${meta.artist}</p>
      </div>
    </div>
  `;
...
function downloadTrack(trackId) {
  ...
  setTimeout(() => {
    hideLoading();
    showNotification(`Track "${meta.title}" downloaded!`, "success");
    logActivity("Track Downloaded", meta.title);
  }, 1500);
}
